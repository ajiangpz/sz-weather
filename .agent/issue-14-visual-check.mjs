...
const screenshots = [];
const validation = { e2e: {}, viewports: [], windFrames: [], pageErrors: [] };
...
try {
  ...
  page.on('pageerror', error => validation.pageErrors.push(error.message));
  ...
  validation.e2e = {
    dashboardVisible: await page.getByRole('region', { name: 'RainScope 深圳天气可视化大屏' }).isVisible(),
    leftPanelVisible: await page.getByRole('complementary', { name: '图层与站点' }).isVisible(),
    rightPanelVisible: await page.getByRole('complementary', { name: '实时指标与预警' }).isVisible(),
    districtLabelCount: labelCount,
    windEnabled: await page.getByLabel('风场流线').isChecked(),
  };

  for (const [width, height] of [[1920, 1080], [1536, 1024], [1440, 900]]) {
    await prepare(width, height);
  ...
  await prepare(1536, 1024);
  const windMap = page.getByRole('region', { name: '深圳降雨雷达地图' });
  const samples = [['0000ms', 0], ['0120ms', 120], ['0240ms', 120], ['0600ms', 360], ['1800ms', 1200]];
