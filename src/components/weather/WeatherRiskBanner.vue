<template>
  <section class="dashboard-panel risk-banner" aria-label="全国气象风险概览">
    <div class="risk-banner__main">
      <span class="risk-banner__icon" aria-hidden="true">!</span>
      <strong>DEMO · {{ title }}</strong>
      <span>{{ affectedCount }} 个区域受影响 · {{ activeAlerts.length }} 条风险提示生效</span>
    </div>
...
const title = computed(() => activeAlerts.value.length > 0 ? '区域气象风险升高' : '当前无生效气象风险');
